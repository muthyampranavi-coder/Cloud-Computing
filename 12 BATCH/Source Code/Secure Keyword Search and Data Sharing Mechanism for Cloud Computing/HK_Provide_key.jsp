...
connection.createStatement().executeUpdate("update patient_details_backup set desc_sk='"+pk+"' where patient_name='"+patient+"' and downer='"+provider+"'");
	...
	connection.createStatement().executeUpdate("insert into hkas_transaction (user,patient,provider,doctor,status) values('Delgatee', '"+patient+"','"+provider+"', '"+doctor+"' ,'"+pk+"')");
	connection.createStatement().executeUpdate("update key_request set sk='"+pk+"' where id='"+id+"'");
		...
		else
		{
			...
			connection.createStatement().executeUpdate("insert into hkas_transaction (user,patient,provider,doctor,status) values('Delgatee', '"+patient+"','"+provider+"', '"+doctor+"(Delegator)' ,'"+sk+"')");
